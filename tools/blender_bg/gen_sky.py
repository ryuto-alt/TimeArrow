...
ステージごとにパレット/時計環/月/星密度/地平シルエットを変えて空気を差別化する:
  ...
  5 最終回廊 = 終焉の蝕。血赤の空+黒い太陽(赤熱コロナ)、稜線は塔の墓場
使い方: python gen_sky.py [n ...]  (番号指定でそれだけ再生成。省略時は全部)
"""
import os
import sys
import numpy as np
...
SKIES = {
    ...
    # 最終ステージ: 終焉の蝕。黒い太陽が沈みかけ、空は血赤、地平は塔の墓場
    5: dict(stops=[(0.00, (16, 5, 10)), (0.38, (52, 12, 18)), (0.66, (120, 30, 26)),
                   (0.80, (214, 84, 38)), (0.86, (66, 24, 22)), (1.00, (26, 10, 12))],
            stars=60, neb=(255, 90, 50), ring=[(0.30, 0.30, 0.42)],
            ring_col=(255, 120, 60), moon=None, eclipse=(0.62, 0.34, 0.14),
            gears=5, sil_h=1.9, shade=(30, 12, 12)),
}
...
def make_sky(n, cfg):
    ...
    for (rcx, rcy, rr) in (cfg["ring"] or []):
        ...
        for ch, c in enumerate(cfg.get("ring_col", (150, 210, 255))):
            img[:, :, ch] += np.clip(ring, 0, 1.4) * c * 0.10

    # 蝕: 黒い太陽(暗い円盤+赤熱コロナ縁)。最終ステージの主役
    if cfg.get("eclipse"):
        ecx, ecy, er = W * cfg["eclipse"][0], H * cfg["eclipse"][1], H * cfg["eclipse"][2]
        dist = np.sqrt((xx - ecx) ** 2 + (yy - ecy) ** 2)
        disc = np.clip(1.0 - (dist / er) ** 8, 0, 1)
        rim = np.exp(-((dist - er) / (er * 0.10)) ** 2)
        halo = np.exp(-np.clip(dist - er, 0, None) / (er * 1.1)) * 0.5
        for ch, c in enumerate((10, 4, 6)):          # ほぼ黒の円盤
            img[:, :, ch] = img[:, :, ch] * (1 - disc) + c * disc
        for ch, c in enumerate((255, 120, 40)):      # 赤熱コロナ
            img[:, :, ch] += c * (rim * 0.85 + halo * 0.35)
    ...
    tex_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "..", "..", "assets", "textures")
    Image.fromarray(out, "RGB").save(os.path.join(tex_dir, f"bg_sky{n}.png"))
    print(f"bg_sky{n}.png written")


targets = [int(a) for a in sys.argv[1:]] or list(SKIES)
for n in targets:
    make_sky(n, SKIES[n])
