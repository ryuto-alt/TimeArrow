...
local function applyVolumes()
  -- マスター既定値は50%(初回起動の爆音防止。ユーザーが触れば保存値が優先)
  audio:setMasterVolume(loadNum("opt_master", 0.5))
...
local function syncWidgets()
  ...
  if ms  and ms:isValid()  then scene:setUiSlider(ms,  loadNum("opt_master", 0.5)) end
