...
local SKIP_TO    = 0.2247 + 16 * (60.0 / 128.0) + 0.05  -- スキップ着地点=サビ頭+0.05s(TitleLogoIntroと一致)
...
local bgmFadeT = 0
local skipped = false  -- イントロスキップ済みフラグ

-- イントロスキップ: 曲も演出もサビ頭へジャンプ(次のシーンには行かない)。
-- UIはスキップ後 bgmT が UI_SHOW_AT に達した時点(約0.9s後)で通常フローのまま出る
local function skipIntro()
  skipped = true
  bgmT = SKIP_TO
  audio:seekBGM(SKIP_TO)
  events:emit("title_skip", {})
end
